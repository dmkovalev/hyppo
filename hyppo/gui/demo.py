...
_EDGES = [
    ...
    ["H11", "H12"],
    ["H12", "H13"],
    ["H13", "H14"],  # ветвь обводнённости
    ["H8", "H16"],
    ["H14", "H16"],  # слияние → OPR
]

_LABELS = {
    "H1": "Агрегация закачки",
    "H2": "CRM быстрый канал",
    "H3": "CRM медленный канал",
    "H4": "Смешение каналов",
    "H5": "Продуктивность",
    "H6": "Первичная добыча (спад)",
    "H7": "ML-коррекция (MLP)",
    "H8": "LPR Fusion (вентиль g)",
    "H9": "Материальный баланс",
    "H10": "Corey krw",
    "H11": "Corey kro",
    "H12": "Баклея–Леверетта fw",
    "H13": "WCT-якорь (физика)",
    "H14": "WCT Fusion",
    "H15": "ГРП (модуляция)",
    "H16": "OPR — прогноз нефти",
}
_BRANCH = {
    **{f"H{i}": "LPR (жидкость)" for i in range(1, 9)},
...
def _build_ve():
    ...
    configuration = [
        ...
        for hid, pmap in _PARAMS.items()
        for ax, lv in pmap.items()
    ]
    return {
        "domain": (
            "Гибридная ёмкостно-резистивная модель — прогноз нефтедобычи "
            "при заводнении (Norne / Brugge)"
        ),
        "ontology": {
            ...
            "classes": [
                ...
                {
                    "name": "Injector",
                    "label": "Нагнетательная скважина",
                    "subclass_of": "Well",
                },
                {
                    "name": "Producer",
                    "label": "Добывающая скважина",
                    "subclass_of": "Well",
                },
