...
def test_superseded_when_competitor_dominates():
    ...
    assert evaluate_status(0.9, own_aic=120.0, best_competitor_aic=100.0) \
...
def test_not_superseded_within_aic_margin():
    ...
    assert evaluate_status(0.9, own_aic=110.0, best_competitor_aic=100.0) \
...
def test_superseded_dominates_even_when_refuted():
    ...
    assert evaluate_status(0.4, own_aic=200.0, best_competitor_aic=100.0) \
        is EpistemicStatus.SUPERSEDED


def test_custom_thresholds():
    assert evaluate_status(0.85, theta_sup=0.9) is EpistemicStatus.REFUTED
    assert evaluate_status(0.95, own_aic=105.0, best_competitor_aic=100.0,
                           theta_aic=2.0) is EpistemicStatus.SUPERSEDED
...
def test_runner_assigns_supported_and_refuted():
    ...
    results = runner.execute(
        ...
        models={"good": lambda c: {"r2": 0.9, "aic": 100.0},
...
def test_runner_assigns_superseded_via_competes():
    ...
    results = runner.execute(
        ...
        models={"h": lambda c: {"r2": 0.85, "aic": 150.0},
