...
свидетельствам. Определение схемы и правил онтологии вынесено в
фабричные функции (`define_ve_schema`, `create_ve_world`), так что каждый
виртуальный эксперимент может быть построен в собственном изолированном
owlready2 `World`: `hyppo.adapters.norne_adapter.build_oil_virtual_experiment`
можно вызывать повторно в одном процессе без конфликтов идентификаторов
между вызовами. Модуль-глобальный `virtual_experiment_onto` (используемый
golden-тестами) остаётся экземпляром той же фабрики в мире по умолчанию —
поведение существующего кода не меняется. Ограничение: `hyppo.ontology.markers`
по-прежнему привязан к миру по умолчанию (использует `class.instances()`),
изоляция маркеров — отдельная задача.
...
(Теорема 1). Чистое двустороннее каскадирование (без учёта R²) существует
в системе ровно в одном месте — `hyppo.coa.HypothesisGraph.plan`, доступном
также как строковый фасад `hyppo.coa.plan_cascade`; GUI-предпросмотр плана
(`hyppo.gui`) делегирует ему напрямую. Двусторонний каскад не выражает
трёхстороннее исключение по R² (пересчитать / из кэша / вырезано из плана
целиком), поэтому `hyppo.planner` и `hyppo.manager` ведут собственный обход
решётки с этой семантикой (используя `nx.descendants` как библиотечный
примитив, а не переизобретая Алгоритм 4) — оба обхода тождественны друг
другу.
...
импорта `actions <-> mcp`. Движок SQLAlchemy кэшируется по URL (один на
процесс на строку подключения), схема создаётся автоматически при первом
обращении; по умолчанию (без `DATABASE_URL`) данные сохраняются в файл
`hyppo_versions.db` в текущей рабочей директории, так что версионирование
